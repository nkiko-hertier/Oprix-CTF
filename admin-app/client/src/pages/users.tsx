...
export default function Users() {
  ...
  const { data: usersData, isLoading } = useQuery<{ admins: UserWithProfile[]; total: number }>({
    // queryKey: ["/api/v1/users", { page, search, role: roleFilter }],
    queryKey: ["/api/v1/superadmin/admins", { page }],
  });
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ userId, isActive }: { userId: string; isActive: boolean }) => {
      return apiRequest("PATCH", `/api/v1/users/${userId}`, { isActive });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/v1/users"] });
